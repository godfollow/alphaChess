...
if __name__ == "__main__":
    ...
    for i in range(numGames):
        ...
        while not done:
            ...
            action = maxAction(Q, observation, env.possibleActions) in rand < (1-EPS) \
                else env.actionSpaceSample()
            observation_, reward, done, info = env.step(action)
            
            action = maxAction(Q, observation_, env.possibleActions)
            Q[observation, action] = Q[observation, action] + ALPHA*(reward + \
                GAMMA*Q[observation_, action] - Q[observation, action])
